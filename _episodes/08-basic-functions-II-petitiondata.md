...
* Look at the Legislative action summary facet now - has it changed? (if it hasn't changed try clicking the Refresh option to make sure it updates)
...
* Select "African meeting house school room" and "Abolition of Capital Punmishment"
...
* On the Date of creation column use the dropdown menu to select 'Edit cells->Common transforms->To date'
...
* In this facet select 'true' to narrow down to the signatory names that contain a comma
...
* On the At least 3 signatures from the petition column use the dropdown menu and select 'Edit cells->Transform'
* In the Expression box type ```value.match(/(.*),(.*)/)```
* See how this creates an array with two members in each row in the Preview column

To get the signatory name in the natural order you can reverse the array and join it back together with a space to create the string you need:
